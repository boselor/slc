...
namespace slc {
    ...
    bool IniConfiguration::loadFile(EString file) {
        return this->readConfig(file.toStdString());
    }

    void IniConfiguration::setLogger(Logger &logger) {
        this->logger = logger;
    }

    bool IniConfiguration::isSpace(char c) {
        if (' ' == c || '\t' == c)
            return true;
        return false;
    }

    bool IniConfiguration::isCommentChar(char c) {
        switch (c) {
            case '#':
                return true;
            default:
    ...
    void IniConfiguration::trim(std::string &str) {
        if (str.empty()) {
            return;
        }
    ...
    bool
    IniConfiguration::analyseLine(const std::string &line, std::string &section, std::string &key, std::string &value) {
        if (line.empty())
            return false;
    ...
    bool IniConfiguration::readConfig(const std::string &filename) {
        settings_.clear();
    ...
    std::string IniConfiguration::readString(const char *section, const char *item, const char *default_value) {
    ...
    int IniConfiguration::readInt(const char *section, const char *item, const int &default_value) {
        std::string tmp_s(section);
        std::string tmp_i(item);
        std::map<std::string, std::string> k_v;
        std::map<std::string, std::string>::iterator it_item;
    ...
    float IniConfiguration::readFloat(const char *section, const char *item, const float &default_value) {
        std::string tmp_s(section);
        std::string tmp_i(item);
        std::map<std::string, std::string> k_v;
        std::map<std::string, std::string>::iterator it_item;
